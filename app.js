...
const dailyChallenges = [
  { title: "Hollow Body Hold", description: "3 Sätze à 30-45s. Der untere Rücken MUSS am Boden bleiben!" },
  { title: "Scapula Pull-Ups", description: "3 Sätze à 10 Wiederholungen. Nur aus den Schultern ziehen, Arme gestreckt." },
  { title: "L-Sit Prep", description: "3 Sätze à 15-20s Tuck L-Sit. Drück die Schultern aktiv nach unten." },
  { title: "Plank Challenge", description: "Maximaler Plank Hold. Wie lange schaffst du heute? (Saubere Form!)" },
  { title: "Dragon Flag Negatives", description: "5 langsame, kontrollierte negative Wiederholungen. Halte die Spannung." },
  { title: "Skin the Cat", description: "3 Sätze à 3 langsame Wiederholungen. Spüre die Dehnung in der Schulter." },
  { title: "Dead Hang", description: "2 Sätze à 60s hängen. Lass die Schultern entspannt aushängen." }
];
...
async function fetchCoachAdvice(message) {
  // Option 1: Local API Key (Client-side request)
  if (state.config.apiKey) {
    ...
    if (!response.ok) throw new Error("Groq API Fehler (Lokal)");
    const data = await response.json();
    return data.choices?.[0]?.message?.content || "Der Coach hat keine Antwort geliefert.";
  }

  // Option 2: Vercel Backend Proxy
  ...
  if (apiBase && !apiBase.includes("<DEIN-VERCEL-PROJEKT>")) {
    ...
    if (!response.ok) throw new Error("Coach API Fehler (Backend)");
    const data = await response.json();
    return data.reply || "Der Coach hat keine Antwort geliefert.";
  }

  // Option 3: Fallback Example
...
async function requestCoachUpdate(reason) {
  const now = Date.now();
  // Don't auto-prompt if no API key is configured (either local or backend)
  // But wait, the user might have backend configured without key in browser (SAFE_API_KEY on server).
  // So we should try if backend URL is valid.

  if (now - lastAutoPromptAt < AUTO_PROMPT_COOLDOWN_MS) {
    return;
  }

  // Basic check if we can even call the coach
  const apiBase = state.config.apiBase || DEFAULT_API_BASE;
  const hasBackend = apiBase && !apiBase.includes("<DEIN-VERCEL-PROJEKT>");
  const hasLocalKey = !!state.config.apiKey;

  if (!hasBackend && !hasLocalKey) {
     // No valid config, don't auto-spam example replies
     return;
  }
