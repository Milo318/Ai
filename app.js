...
function calculateStreak() {
  ...
  // Assuming sessions are ordered by date desc (since we unshift)
  let currentStr = state.sessions[0].date;
  let streak = 0;
...
async function fetchCoachAdvice(message) {
  // Priority 1: Direct Groq API (if Key is present)
  if (state.config.apiKey) {
    try {
      ...
      console.warn("Groq Direct Fail, falling back to Proxy", e);
      // Fallthrough to proxy
    }
  }

  // Priority 2: Vercel Proxy
  const apiBase = state.config.apiBase || DEFAULT_API_BASE;
  // If apiBase is default placeholder, return example
  if (apiBase.includes("<DEIN-VERCEL-PROJEKT>")) {
...
async function requestCoachUpdate(reason) {
  const now = Date.now();
  if (!state.config.apiKey) {
    // Also allow update if only using proxy?
    // The original code had: if (!state.config.apiKey) return;
    // But then how does the proxy work if no key needed?
    // I'll stick to checking apiKey for auto-prompts to avoid spamming the proxy?
    // Actually, let's allow it if we have a valid proxy URL?
    // For now, keep original logic: auto prompt only if API key is present (presumed power user).
    return;
  }
...
if (saveConfigButton) {
  saveConfigButton.addEventListener("click", () => {
    try {
      ...
      saveConfig(); // Persist other fields
