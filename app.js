...
function saveConfig() {
  state.config.apiBase = apiBaseInput.value.trim();
...
async function fetchCoachAdvice(message) {
  ...
  if (apiBase && !apiBase.includes("<DEIN-VERCEL-PROJEKT>")) {
    ...
    if (!response.ok) {
        ...
        throw new Error(data?.error || "Coach API Fehler (Backend)");
    }
    const data = await response.json();
    return data.reply || "Der Coach hat keine Antwort geliefert.";
  }

  // 2. Try Direct Groq if API Key is configured
  if (apiKey) {
      const prompt = [
          ...
          `Kontext: ${JSON.stringify(buildContext())}`,
      ].join(" ");
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data?.error?.message || "Coach API Fehler (Groq)");
      }
      const data = await response.json();
...
async function requestCoachUpdate(reason) {
  const now = Date.now();
  // Only auto-update if we have a config (don't spam demo mode unless user asked)
  if (!state.config.apiBase && !state.config.apiKey) {
    return;
  }
...
if (coachForm) {
  coachForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      console.error(error);
      pushMessage(`Fehler: ${error.message}`, "coach");
